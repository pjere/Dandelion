"""Reproducibility metadata (§6.5, §8): stamp outputs with the hashes that make a run bit-reproducible
from (inputs, config, seed)."""
...
def run_metadata(config: Config, weather_draw: str | int | None = None,
    ...
    return {
        ...
        "weather_cube_hash": _file_hash(config.resolve(config.section("data")["weathergen_output"])),
        "weather_draw": str(weather_draw) if weather_draw is not None else "0",
        "seed": int(seed if seed is not None else config.seed),
        "availability_model_version": __import__("availability_model").__version__,
    }
