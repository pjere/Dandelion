"""avail-model CLI: init-workbook, calibrate, project, validate.
...
def cmd_init_workbook(args):
    ...
    build_template(cfg, out)
    print(f"[init-workbook] reference template -> {out}\n"
          "  The live source is the merged ../scenarios.xlsx; copy tabs across with an 'avail_' prefix.")
...
def cmd_project(args):
    ...
    project(load_config(args.config), n_draws=args.draws)
...
def main(argv: list[str] | None = None) -> None:
    p = argparse.ArgumentParser(prog="avail-model", description=__doc__)
    ...
    sub.add_parser("calibrate", help="infer outage history + fit availability").set_defaults(func=cmd_calibrate)
    pj = sub.add_parser("project", help="simulate unit availability from the workbook + weather draws")
    pj.add_argument("--draws", type=int, default=None, help="override projection.n_draws (e.g. for a quick run)")
